...
describe("useGridKeyboardNavigation", () => {
  ...
  it("should set the active index to 0 when focusing for the first time", () => {
    ...
    // testing this with a real component is difficult, so we only test the callback
    act(() => result.current.onFocus(new Event('focus')));
  ...
  it("should consider the navigation direction when focusing the element with a custom event", () => {
    ...
    // testing this with a real component is difficult, so we only test the callback
    act(() => result.current.onFocus(new CustomEvent('focus', { detail: { keyboardDirection: NAV_DIRECTIONS.LEFT }})));
