...
export const ELEMENT_WIDTH_PX = 30;
...
<Canvas>
  <Story name="Overview">
    ...
    return (
      <div style={{ padding: PADDING_PX, width, display: 'flex', flexWrap: 'wrap' }} ref={ref} tabIndex={-1} onBlur={onBlur} onFocus={onFocus}>
        {items.map((item, index) => (
          <div key={item} onClick={onClickByIndex(index)} style={{width: ELEMENT_WIDTH_PX}} className={cx("item", { "active-item": index === activeIndex })}>
            {item}
          </div>
        ))}
      </div>
